...
** Intro 
ISO/IEC 9126 defines a quality model
- External & Internal Quality :
  - Functionl  
    - [Suitablity, Accuracy,Interoperability,Security Functionality Conmpliance]
  - Reliablity 
    - [Maturity, Fault Tolerance, Recoverability, Reliability Compliance]
  - Usability 
    - [Understandability Learnability, Operability,Attractivenss, Usability Compilance]
  - Efficiency 
    - [Time Behaviour, Resource Utillisation, Efficiency Compliance]
  - Maintanablity 
    - Analysability, Changeability, Stability, Testability, Maintainability Compilance 
  - Portability 
    - Adaptability, Installability, Co-Existence, Replaceability, Portability Compilance
** What is SonarQube?
...
Sonar uses various static & dynamic code analysis tools such as Checkstyle, PMD, FindBugs, FxCop, Gendarme, and many more to extract software metrics, which then can be used to improve software quality. Provides lots of 
https://604223-1956433-raikfcquaxqncofqfm.stackpathdns.com/wp-content/uploads/2015/10/sonar_structure_2016-01-26_21-32-53.png

** Sonar CI
https://604223-1956433-raikfcquaxqncofqfm.stackpathdns.com/wp-content/uploads/2015/10/sonar_CI_2016-01-26_21-33-42.png
...
** Installation 
linux installation https://devopscube.com/setup-and-configure-sonarqube-on-linux/
#+begin_src sh
...
sysctl vm.max_map_count # Step 4: Login as root and execute the following commands.
sysctl fs.file-max
ulimit -n
ulimit -u

# Setup PostgreSQL 10 Database For SonarQube
sudo yum install https://download.postgresql.org/pub/repos/yum/10/redhat/rhel-7-x86_64/pgdg-centos10-10-2.noarch.rpm -y # Step 1: Install PostgreSQL 10 repo.
sudo yum install postgresql10-server postgresql10-contrib -y # Step 2: Install PostgreSQL 10
sudo /usr/pgsql-10/bin/postgresql-10-setup initdb # Step 3: Initialize the database.
sudo vi /var/lib/pgsql/10/data/pg_hba.conf # Step 4: Open /var/lib/pgsql/data/pg_hba.conf file to change the authentication to md5.
...
# Configur Sonar User and Database
# Change the default password
sudo passwd postgres
# change passwd
# Login postres user with new password
su -postgres
psql

# EXECUTE FOLLOWING CMD IN PSQL
...
#+begin_src sh
# Step 1: Download,unzip,install in /opt 
cd /opt 
...
sudo vi /opt/sonarqube/conf/sonar.properties #  You will find jdbc parameter under PostgreSQL section.
  #sonar.jdbc.username=sonar                                                                                                                     
  #sonar.jdbc.password=sonar-db-password
  #sonar.jdbc.url=jdbc:postgresql://localhost/sonar
  #sonar.web.host=0.0.0.0
  #sonar.web.port=80  # default sonar run on 9000
  #sonar.web.context=/sonar #If you want to access sonarqube some path like http://url:/sonar, change the following parameter.
...
./sonar.sh start # start sonarserver
sudo ./sonar.sh status # check status

# Setting up Sonarqube as a service
sudo vi /etc/systemd/system/sonarqube.service #copy paste below lines
############################################################3
[Unit]
